...
All metric implementations live in [`scripts/lib/qa_metrics.py`](https://github.com/ianlintner/ai-pixel-art-image-generation/blob/main/scripts/lib/qa_metrics.py). Thresholds are declared in the `GATES` dict at the bottom of that file.
...
The CI workflow [`ci.yml`](https://github.com/ianlintner/ai-pixel-art-image-generation/blob/main/.github/workflows/ci.yml) runs this on every bundled example artifact so the committed examples are guaranteed to pass.
